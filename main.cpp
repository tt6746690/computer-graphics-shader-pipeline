...
Usage:
    ...
    int _height)
  {
    ...
    // note near and far are in the positive z-axis
    float nearVal = 0.01;
    float farVal = 100;
    float top = tan(35./360.*M_PI)*nearVal;   // vertical field-of-view of 70 degrees
    float right = top * (double)::width/(double)::height;
    float left = -right;
    float bottom = -top;

    // perspective projection is updated when window size is updated
  ...
  while (!glfwWindowShouldClose(window))
  {
    ...
    // specifies the value of a "uniform" variable for current program
    glUniformMatrix4fv(
      // returns location of a uniform location
    ...
    // double buffer: one for displaying; one for rendering for next frame
    glfwSwapBuffers(window);

    // 60 fps
    // higher fps will not give better result and increases latency
    {
      // event processing is usually done after swapping the buffers
      glfwPollEvents();
      // In microseconds
